"""
Evaluacija V3 modela (trening sa augmentacijom suma) na cistom test skupu.

Ucitava prag odluke iz outputs/reports/threshold_tuning_v3.json ako postoji,
inace koristi podrazumijevani prag 0.5.

Sprema:
  outputs/reports/eval_clean_v3.json
  outputs/figures/confusion_matrix_clean_v3.{png,pdf}
  outputs/figures/roc_clean_v3.{png,pdf}
"""
...
parser = argparse.ArgumentParser(description="Evaluacija V3 modela na cistom test skupu.")
parser.add_argument("--model-path", type=str, default=str(DEFAULT_MODELS_PATH / "best_model_v3.pt"))
parser.add_argument("--output-json", type=str, default=str(DEFAULT_REPORTS_PATH / "eval_clean_v3.json"))
...
if args.threshold is not None:
    ...
    print(f"Prag odluke ucitan iz {TUNING_FILE}: {threshold:.2f}")
else:
    threshold = 0.5
...
# ---- Evaluacija ----
print(f"Pokretanje evaluacije (prag={threshold:.2f}) ...")
results = evaluate(model, test_loader, threshold=threshold)
...
# ---- Ispis ----
m = report["metrics"]
print("\nRezultati V3 modela na cistom test skupu:")
...
if not args.no_figures:
    ...
    ax_cm.set_title("V3 - Matrica konfuzije - cist test skup", fontweight="bold")
    fig_cm.tight_layout()
    for ext in ("png", "pdf"):
        p = FIGURES_PATH / f"confusion_matrix_clean_v3.{ext}"
    ...
    RocCurveDisplay.from_predictions(
        ...
        name=f"V3 CNN (AUC = {m['auc_roc']:.3f})",
        ax=ax_roc, color="#0072B2",
    )
    ax_roc.plot([0, 1], [0, 1], "k--", lw=0.8, label="Slucajni klasifikator")
    ax_roc.set_title("V3 - ROC kriva - cist test skup", fontweight="bold")
    ax_roc.legend(loc="lower right")
    fig_roc.tight_layout()
    for ext in ("png", "pdf"):
        p = FIGURES_PATH / f"roc_clean_v3.{ext}"
