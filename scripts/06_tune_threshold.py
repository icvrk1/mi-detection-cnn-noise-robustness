...
Runs inference on val.npz once, then sweeps decision thresholds
from 0.30 to 0.70 (step 0.01) and picks the one that maximises F1.
Model weights are never modified.

Saves:
  outputs/reports/threshold_tuning.json
  outputs/figures/thesis/fig_threshold_sweep.{pdf,png}
"""
...
# Single inference pass — collect probabiliti
...
ax.set_title("F1 mjera na validacijskom skupu u zavisnosti od praga odluke",
             fontweight="bold")
...
for ext in ("pdf", "png"):
    p = FIGURES_PATH / f"fig_threshold_sweep.{ext}"
